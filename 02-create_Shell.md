...
Let's go back to Simone's home directory,
`/Users/simone`,
...
and then move up one directory to `/Users/simone` using `cd ..`:
...
> mean "the current user's home directory". For example, if Simone's home
> directory is `/Users/simone`, then `~/data` is equivalent to
> `/Users/simone/data`. This only works if it is the first character in the
> path: `here/there/~/elsewhere` is *not* `/Users/simone/elsewhere`.
